...
export default function AuditHubReconciliationPage() {
  ...
  return (
    <div className="space-y-4">
      <p className="text-sm text-[#7F8C8D]">{t("auditHub.reconHint")}</p>
      <div className={`${CARD_CONTAINER_CLASS} grid gap-3 sm:grid-cols-2`}>
        ...
        <label className="text-xs font-semibold text-[#34495E] sm:col-span-2">
          ...
          <input
            className={`${INPUT_BORDERED_CLASS} mt-1 w-full max-w-xs`}
            value={take}
            onChange={(e) => setTake(e.target.value)}
          />
        </label>
        <label className="flex items-center gap-2 text-xs font-semibold text-[#34495E] sm:col-span-2">
          <input
            type="checkbox"
            checked={includeTotals}
            onChange={(e) => setIncludeTotals(e.target.checked)}
          />
          <span>{t("auditHub.reconIncludeTotals")}</span>
        </label>
      </div>
