...
export default function DataHubMdmCompaniesPage() {
  ...
  const load = useCallback(async () => {
    ...
    const res = await apiFetch(`/api/admin/mdm/companies?${params}`);
    if (!res.ok) {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title={t("superAdmin.dataHubMdmCompanies")} subtitle={t("superAdmin.dataHubReadOnly")} />
      ...
      <p className="text-[13px] text-[#7F8C8D]">
        {t("superAdmin.totalLabel", { count: total })}
      </p>
