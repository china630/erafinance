...
export default function DataHubNasChartPage() {
  ...
  const patchDeprecate = async (id: string, isDeprecated: boolean) => {
    ...
    setBusyId(id);
    const res = await apiFetch(`/api/admin/chart-template/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-4">
      <PageHeader title={t("superAdmin.dataHubNasChart")} subtitle={t("superAdmin.chartTemplateIntro")} />
      ...
      <form onSubmit={(e) => void saveRow(e)} className={`${CARD_CONTAINER_CLASS} p-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-9`}>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className={DATA_TABLE_TR_CLASS}>
                <td className={`${DATA_TABLE_TD_CLASS} text-xs whitespace-nowrap`}>{r.kind}</td>
                ...
                <td className={`${DATA_TABLE_TD_CLASS} text-right text-xs`}>{r._count?.accounts ?? 0}</td>
                <td className={`${DATA_TABLE_TD_CLASS} text-center text-xs`}>{r.isDeprecated ? "✓" : "—"}</td>
                <td className={DATA_TABLE_TD_CLASS}>
                  <button
                    type="button"
                    className={r.isDeprecated ? PRIMARY_BUTTON_CLASS : SECONDARY_BUTTON_CLASS}
