/**
 * CLI entry point: `tsx prisma/import-banks-md.ts [--dry-run] [--file=path]`
 *
 * Default `--file` path is `<repo-root>/docs/banks.md`. The script always
 ...
 *   npm run db:import-banks-md -- --file=docs/banks.md
...
function parseArgs(argv: string[]): ParsedArgs {
  let dryRun = false;
  let file = resolve(__dirname, "..", "..", "..", "docs", "banks.md");
  ...
  for (const raw of argv.slice(2)) {
    if (raw === "--dry-run" || raw === "-n") {
      ...
      console.info(
        "Usage: tsx prisma/import-banks-md.ts [--dry-run] [--file=PATH] [--voen=A,B]",
      );
      process.exit(0);
