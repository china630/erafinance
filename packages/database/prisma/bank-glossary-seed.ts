 ...
 *
 * The list is the authoritative source of truth (TZ §14.0.2): VÖEN, SWIFT,
 * correspondent IBAN, head-office phones and address are the official records
 * of the Central Bank of Azerbaijan; branch trees are imported separately
 * from `docs/banks.md` via `npm run db:import-banks-md`.
 */
export interface BankGlossarySeedRow {
  /** Two-character platform code, `01`–`22`. */
  code: string;
...
export const BANK_GLOSSARY_SEED: ReadonlyArray<BankGlossarySeedRow> = [
  ...
  },
  {
    ...
    headAddress:
      "Bakı şəhəri, Yasamal r-nu, İ. Qutqaşınli küçəsi, ev 112",
  },
  {
    code: "19",
  ...
  },
  {
    ...
    headPhones: [],
    headAddress:
      "Bakı şəhəri, Yasamal rayonu, Cəfər Cabbarlı küçəsi, 40",
  },
  {
    code: "22",
 ...
 * redeploy.
 *
 * Two-phase write protects the `BankGlossary.voen UNIQUE` constraint when
 * VÖEN values shuffle between codes (e.g. when a new release re-numbers the
 * platform `code` slots).
 *
  ...
  await prisma.$transaction(async (tx) => {
    ...
    for (const row of existing) {
      await tx.bankGlossary.update({
        where: { id: row.id },
        data: { voen: `__tmp_${row.code}_${row.id.slice(0, 8)}` },
      });
    }
